...
Current release evidence: GenVM semantic/schema checks pass for the unchanged deployed contract; Python `20/20`; frontend `14/14`; production build passes. The frontend regressions cover EIP-6963/legacy deduplication, provider-native Studionet switch/add/readback without MetaMask Snap APIs, disconnected startup on every reload until the user opens the selector, receipt-envelope reconciliation, and automatic editable snapshot labeling. See [verification](docs/VERIFICATION.md).
